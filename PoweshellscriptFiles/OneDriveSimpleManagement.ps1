# OneDriveSimpleManagement統合スクリプト
# 機能：OneDriveの使用状況を取得し、各種形式でレポートを出力します
...
Write-Host "現在のユーザ種別: $userType"

# 一般ユーザなら管理者の承認をリクエスト
if ($userType -eq "Member" -and -not $isAdmin) {
    Write-Host "一般ユーザのため、API パーミッションの付与が必要です。"
...
if ($isAdmin) {
    ...
    try {
        ...
        Write-Output "自分のユーザー種別: $userTypeValue"
...
function colorizeRows() {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        if (oneDriveStatusCell) {
            ...
            } else if (status.includes("90%以上")) {
                rows[i].classList.add('danger');
            } else if (status.includes("70%以上")) {
